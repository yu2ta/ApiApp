    ...
    private fun updateFavoriteItemViewHolder(holder: FavoriteItemViewHolder, position: Int) {
        ...
        holder.apply {
            rootView.apply {
                ...
                setOnClickListener {
                    val cpu : CouponUrls = CouponUrls(items[position].url, items[position].url)
                    val shop: Shop = Shop(items[position].id, "", cpu,items[position].imageUrl,items[position].name)
